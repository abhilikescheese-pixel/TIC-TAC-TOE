...
def check_winner():
    ...
    if (turns == 9):
        game_over = True
        label.config(text="Game is a tie!", foreground=color_yellow)
...
label = tkinter.Label(frame, text=current_player+"'s Turn", font=("Consolas", 20), background=color_grey,
                      foreground="white")
...
button = tkinter.Button(frame, text="Reset", font=("Consolas", 20), background=color_grey, 
...
window_x = int((screen_width/2) - (windowwidth/2))
window_y = int((screen_height/2) - (windowheight/2))

window.geometry(f"{windowwidth}x{windowheight}+{window_x}+{window_y}")
